...
//#region src/mcp-discovery.ts
const run = promisify(execFile);
function isDockerGateway(config) {
	return path.basename(config?.command ?? "") === "docker" && config?.args?.slice(0, 3).join(" ") === "mcp gateway run";
}
function option(args, name) {
	const inline = args.find((arg) => arg.startsWith(`${name}=`));
	if (inline !== void 0) return inline.slice(name.length + 1);
...
function childrenFromProfiles(value, profile, namespace, liveTools) {
	...
	return selected.servers.map((value) => {
		...
		const name = typeof server.name === "string" ? server.name : typeof item.name === "string" ? item.name : typeof item.image === "string" ? item.image : "Unnamed server";
		const tools = Array.isArray(server.tools) ? server.tools.map(record).map((tool) => tool.name).filter((name) => typeof name === "string") : [];
		return {
			name,
...
async function discoverDockerChildren(config, namespace, liveTools, warnings) {
	const args = config.args ?? [];
	const profile = option(args, "--profile");
	if (!profile) {
		const servers = option(args, "--servers");
		if (servers) return servers.split(",").filter(Boolean).map((name) => ({
			name,
			description: "",
			tools: [],
			availableTools: []
...
function slugifySkillName(raw) {
	return raw.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 64).replace(/-+$/g, "");
}
/**
...
*/
function deriveSkillFromSource(content, fileName) {
	const { data, body } = parseSkillDoc(content);
	const h1 = /^#\s+([^\n]+)$/m.exec(body)?.[1]?.trim() ?? "";
	const rawName = typeof data.name === "string" && data.name.trim() !== "" ? data.name : h1 !== "" ? h1 : fileName.replace(/\.md$/i, "").replace(/[-_]+/g, " ");
	const name = isValidSkillName(rawName) ? rawName : slugifySkillName(rawName);
	const description = typeof data.description === "string" ? data.description.trim() : "";
	const whenToUse = typeof data.whenToUse === "string" && data.whenToUse !== "" ? data.whenToUse : void 0;
...
function defaultSkillRoots() {
	...
	return [.../* @__PURE__ */ new Set([path.join(resolveDshHome(), "skills"), path.join(agentsHome, "skills")])];
}
...
function apply(ctx, config) {
	...
	const collectLive = async (warnings) => {
		const toolNames = ctx.get("tools")?.schemas().map((tool) => tool.name) ?? [];
		...
		for (const entry of loader.entries()) {
			...
			out.push({
				...
				children: isDockerGateway(entry.options.config) ? await discoverDockerChildren(entry.options.config, serverName, toolNames, warnings) : [],
				present: true
			});
		}
		return out;
	...
	const receiver = {
		...
		async addSkill(input) {
			const meta = input.sourceFile !== void 0 ? deriveSkillFromSource(input.sourceFile.content, input.sourceFile.name) : void 0;
			...
			try {
				...
				let content;
				if (input.sourceFile !== void 0 && meta !== void 0) {
					...
					const whenToUse = whenToUseInput !== void 0 && whenToUseInput !== "" ? whenToUseInput : meta.whenToUse;
					...
					const safeWhenToUse = whenToUseInput !== void 0 && whenToUseInput !== "" ? whenToUseInput : void 0;
		...
		async previewSkillUpload(source) {
...
//#endregion
export { Config, apply, buildSkillDoc, deriveSkillFromSource, inject, name, parseSkillDoc, planPatch, setSkillInvocation, slugifySkillName, validateServerSet };
